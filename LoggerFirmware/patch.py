  ...
  def parse(self, stream):
    ...
    while fe.next():
      ...
      if hunkbody:
        ...
        if line.strip(b"\r\n") == b"":
            debug("expanding empty line in a middle of hunk body, line_num: %d" % (lineno,))
            self.warnings += 1
            line = b' ' + line
        ...
        if re.match(b"^[- \\+\\\\]", line):
            ...
            warning("invalid hunk no.%d at %d for target file %s, line: |%s|" % (nexthunkno, lineno+1, p.target, line))
